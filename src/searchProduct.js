...
export function getNewArrivals(start, end) {
	...
	return newArrivals.slice(start, end + 1);
}
...
export function getBestSellers(start, end) {
	...
	return bestSellers.slice(start, end + 1);
}
...
export function getComboAssembling(start, end) {
	...
	const comboAssembling = productsData.filter((product) =>
		product.tags.includes("combo")
	);
	return comboAssembling.slice(start, end + 1);
}
...
export function getMQASupport(start, end) {
	...
	const mqaSupport = productsData.filter((product) =>
		product.tags.includes("mqa")
	);
	return mqaSupport.slice(start, end + 1);
}
