		...
		if (slider) {
			const timer = setTimeout(() => {
				...
				const enableImageZoom =
					container.dataset.enableImageZoom === "true";
				...
				addEventDelegate({
					...
					handler: (e, media) => {
						e.preventDefault();
						const isImage =
								media.classList.contains("media-type-image"),
							isZoomButton = e.target.closest(
								selectors.mediaZoomIns[0]
							);
				...
				<div className="preview__wrapper | mb-4 flex-grow">
					<Swiper
						...
						{item.media.map((media, index) => (
							<SwiperSlide
								key={index}
								className="swiper-slide prod-media-item media-type-image"
								data-index={index}
