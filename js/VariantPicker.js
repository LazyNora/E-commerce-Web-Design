...
if (variantId) {
	currentVariant = variantData.find((variant) => parseInt(variant.id) === parseInt(variantId)) || variantData[0];
} else {
...
const updateBrowserHistory = () => {
	!currentVariant || window.history.replaceState({}, "", `${currentPath + productData.url}/?variant=${currentVariant.id}`);
