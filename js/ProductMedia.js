    ...
    <div class="preview__wrapper | mb-4 flex-grow">
      <div class="swiper media-${gallery.getAttribute("type")}">
        <div class="swiper-wrapper">
          ${product.media
				.map(
					(media, index) =>
						`
            <div class="swiper-slide prod-media-item media-type-image" data-index=${index} data-media-type=${media.media_type} data-media-id=${media.id} data-aspect-ratio="1.499">
              ${
              ...
              <div class="prod-media media-image" data-media-id=${media.id} data-media-width=${media.width} data-media-height=${media.height} data-media-alt=${media.alt} data-media-src=${media.src}>
                <div class="prod-image" style="aspect-ratio: 1.499">
                  <img class="img-loaded" src=${media.src + "&width=946"} sizes="946px" loading="lazy" width="946" height="631" alt=${media.alt} fetchpriority="auto">
                </div>
              </div>
              <div class="zoom-in | transition-all opacity-0 absolute z-10 right-5 ${gallery.getAttribute("type") === "desktop" ? "top-5" : "bottom-2.5"}">
                <button class="prod-media__zoom-in | tooltip-item btn-icon tooltip-left tooltip-style-1" type="button" data-product-handle=${product.handle}>
                  ...
                  <span class="tooltip-content" data-revert-text="">
                    Zoom in
                  </span>
                </button>
              </div>
        ...
        <div class="absolute z-10 pointer-events-none inset-x-0 top-1/2 -translate-y-1/2 flex items-center justify-between px-4">
          ...
          <button class="swiper-button-control swiper-button-next btn-icon">
            <svg
              fill="currentColor"
              width="14px"
              height="14px"
            ...
            <div class="swiper-slide" data-index=${index}>
              <div class="prod-media media-image" data-media-id=${media.id} data-media-width=${media.width} data-media-height=${media.height} data-media-alt=${media.alt} data-media-src=${media.src}>
                <div class="prod-image">
                  <img src=${media.preview_image.src + "&width=175"} alt=${media.alt} loading="lazy" class="img-loaded" width="179" height="120" sizes="175px"/>
                </div>
              </div>
            </div>
            `
